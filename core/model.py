...
class StyleEncoder(nn.Module):
    def __init__(self, img_size=256, style_dim=64, num_domains=2, max_conv_dim=512):
        ...
        self.unshared1 = nn.ModuleList()
        self.unshared2 = nn.ModuleList()
        for _ in range(num_domains):
            self.unshared1 += [nn.Linear(dim_out, style_dim)]
        self.unshared2 += [nn.Linear(style_dim*3,  num_domains)]
    ...
    def forward(self, x, y):
        ...
        out1 = []
        out2 = []
        for layer1 in self.unshared1:
            out1 += [layer1(h)]
        out1 = torch.stack(out1, dim=1)  # (batch, num_domains, style_dim)
        idx = torch.LongTensor(range(y.size(0))).to(y.device)
        
        #pdb.set_trace()
        s = out1[idx, y]  # (batch, style_dim)
        o = self.unshared2[0](out1.view(out1.size(0), -1))
        return s, o
...
def build_model2(args):
    ...
    nets = Munch(style_encoder=style_encoder)
    nets_ema = Munch(style_encoder=style_encoder_ema)

    return nets, nets_ema

def build_model3(args):
    discriminator = nn.DataParallel(Discriminator(args.img_size, args.num_domains))
    
    nets = Munch(discriminator=discriminator)
